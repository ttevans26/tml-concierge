...
import { CalendarDays, X, Clock, ArrowRight, Check } from "lucide-react";
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-ink/20 backdrop-blur-sm" onClick={handleClose} />
          <div className="relative w-full max-w-2xl bg-background border border-border rounded-xl shadow-2xl animate-fade-in overflow-hidden">
            {/* Header */}
                ...
                <div className="p-5 flex flex-col items-center">
                  ...
                  <Calendar
                    ...
                    classNames={{
                      ...
                      nav_button:
                        "h-7 w-7 bg-transparent border border-border rounded-md p-0 opacity-60 hover:opacity-100 transition-opacity inline-flex items-center justify-center",
                      nav_button_previous: "absolute left-0",
                      nav_button_next: "absolute right-0",
                      table: "w-full border-collapse",
                      head_row: "flex",
                      head_cell:
                        "text-muted-foreground rounded-md w-9 font-body font-normal text-[0.7rem] uppercase tracking-wider",
                      row: "flex w-full mt-1",
                      cell: "h-9 w-9 text-center text-sm p-0 relative focus-within:relative focus-within:z-20",
                      day: "h-9 w-9 p-0 font-body font-normal text-foreground hover:bg-secondary rounded-md transition-colors inline-flex items-center justify-center",
                      day_selected:
                        "bg-forest text-primary-foreground hover:bg-forest hover:text-primary-foreground focus:bg-forest focus:text-primary-foreground",
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-auto">
                        {TIME_SLOTS.map((slot) => (
                          <button
                            ...
                            className={cn(
                              "flex items-center gap-2 px-3 py-2.5 rounded-md border text-sm font-body transition-all text-left",
                              selectedTime === slot
                      ...
                      {selectedTime && (
                        <div className="mt-5 pt-5 border-t border-border animate-fade-in">
                          ...
                          <textarea
                            ...
                            rows={2}
                            className="w-full bg-secondary border-none rounded-md px-3.5 py-2.5 text-sm font-body text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-forest resize-none"
                          />
                          <button
                            onClick={handleConfirm}
                            className="mt-3 w-full flex items-center justify-center gap-2 bg-forest text-primary-foreground py-2.5 rounded-md text-sm font-body font-medium hover:opacity-90 transition-opacity"
                          >
                            Confirm Booking
